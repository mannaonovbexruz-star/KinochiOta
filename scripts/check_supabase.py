...
def key_role(key: str) -> str:
    """JWT ichidagi `role` ni o'qiydi — KALITNING O'ZI hech qayerga chiqmaydi."""
    try:
        payload = key.split(".")[1]
        ...
        return "o'qib bo'lmadi (JWT emas?)"
...
def main() -> None:
    ...
    from database.client import get_client  # config tekshirilgandan keyin
    ...
    for table in ("movies", "admins"):
        try:
            resp = client.table(table).select("*", count="exact").limit(3).execute()
    ...
    try:
        resp = client.table("movies").select("*").eq("movie_code", "1").limit(1).execute()
        rows = resp.data or []
